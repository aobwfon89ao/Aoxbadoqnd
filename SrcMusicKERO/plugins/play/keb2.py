...
REPLY_MESSAGE_BUTTONS = [
    [
        ("✭ 𝙿𝚊𝚗𝚍𝚊 𝚂𝚘𝚞𝚛𝚌𝚎 𝙺𝚎𝚎𝚙 𝚢𝚘𝚞𝚛 𝚍𝚎𝚊𝚛 𝚒𝚗 𝚝𝚑𝚎 𝚜𝚎𝚌𝚝𝚒𝚘𝚗 ✭"),
    ],
    [
        ("✭ قسم الاذاعه"),
        ("✭ تحكم الحساب المساعد"),
...
def reply_to_HEY(Client, message):
    message.reply_photo(
        photo=f"https://telegra.ph/file/3f01d70611d53c62bb3ce.jpg",
        caption=f"""[ َِ.سـورس ميـوزك الـحيتان.〙-𓏺Whoever humbles #himself to god will be #exalted](https://t.me/R_Q_R7)""",
        reply_markup=InlineKeyboardMarkup(
            [
            [
                InlineKeyboardButton("𓆩 🔻︙قـنـاة الـسـورس︙🔺️ 𓆪", url=f"https://t.me/R_Q_R7"),
            ]
         ]
     )
  )
...
def forward(client: Client, message: Message):
  ...
  if message.from_user.id == 6920192910:
   app.send_message(chat_id," • رتبتك هي : مطور السورس")
  if message.from_user.id == 1654079942:
   app.send_message(chat_id," • رتبتك هي : مبرمج السورس")
...
async def italy(client: Client, message: Message):
    ...
    async for photo in client.iter_profile_photos(message.from_user.id, limit=1):
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("𝙿𝚊𝚗𝚍𝚊 𝚂𝚘𝚞𝚛𝚌𝚎", url=f"https://t.me/R_Q_R7"),
                ],
            ]
        ),
    )
...
async def italy(client: Client, message: Message):
    ...
    async for photo in client.iter_profile_photos(message.from_user.id, limit=1):
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("𝙿𝚊𝚗𝚍𝚊 𝚂𝚘𝚞𝚛𝚌𝚎", url=f"https://t.me/Z_2_X_7"),
                ],
            ]
        ),
    )
...
@app.on_message(filters.regex("༺┉⊶﴿♡ 𝙼𝚞𝚜𝚒𝚌 𝙿𝚊𝚗𝚍𝚊 𝙺𝚎𝚎𝚙 ♡﴾⊷┉༻") & filters.private & SUDOERS)
async def italy(client: Client, message: Message):
    usr = await client.get_users(message.from_user.id)
    name = usr.first_name
    async for photo in client.iter_profile_photos(message.from_user.id, limit=1):
